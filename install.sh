...
sudo apt-get update
sudo apt-get install neovim python3 python3-pip python3-venv git curl exuberant-ctags -y
...
nvim -c ':PlugInstall' -c ':UpdateRemotePlugins' -c ':qall' -c ':CocInstall coc-python'
rm ~/.config/nvim/init.vim

# Symlink init.vim in current working directory to nvim's config location ...
echo '[*] Creating symlink init.vim -> ~/.config/nvim/init.vim'
ln init.vim ~/.config/nvim/

echo '[*] Creating symlink coc-config.json -> ~/.config/nvim/coc-config.json'
ln coc-config.json ~/.config/nvim/
